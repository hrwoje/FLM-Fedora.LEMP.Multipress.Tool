...
# LEMP Stack + phpMyAdmin + WordPress Multisite TOP OPTIMIZATION Script v2.15.1-EN for Fedora
# Author: H Dabo (Concept & Base) / AI (Implementation & Refinement) - 2025
# ------------------------------------------------------------------------------------
# - FIX: Menu readability and alignment with color codes (v2.15.1)
# --- Previous changes incorporated (translated base v2.14.2) ---
...
# - REMOVED: Redis/Valkey detection and installation
# - ADDED: Focus on APCu for Object Caching (requires WP plugin activation)
# - OPT: Nginx client_max_body_size added
# - OPT: Basic MariaDB (InnoDB) optimizations via separate config file
# - OPT: Comprehensive PHP extensions & OPcache + comments for further tuning
...
WORDPRESS_ROOT="/var/www/wordpress"; WP_CONFIG_PATH="${WORDPRESS_ROOT}/wp-config.php"; WP_CONTENT_DIR="${WORDPRESS_ROOT}/wp-content"; NGINX_CONF_DIR="/etc/nginx/conf.d"; PHP_INI_PATH="/etc/php.ini"; PHP_OPCACHE_CONF_PATH="${PHP_EXTENSIONS_DIR}/99-wp-optimized-opcache.ini"; PHP_APCU_CONF_PATH="${PHP_EXTENSIONS_DIR}/40-apcu.ini"; PHP_FPM_WWW_CONF="/etc/php-fpm.d/www.conf"; PHPMYADMIN_CONFIG="/etc/phpMyAdmin/config.inc.php"; PHPMYADMIN_TMP_DIR="/var/lib/phpmyadmin/tmp"; MARIADB_DATA_DIR="/var/lib/mysql"; MARIADB_OPT_CONF="/etc/my.cnf.d/99-wordpress-optimizations.cnf"; LOG_FILE="/var/log/lemp_wp_ms_optimized_apcu_install_en.log"; WP_CLI_PATH="/usr/local/bin/wp" # Adjusted Log filename
...
toggle_multisite() {
    log_message "INFO" "Starting Multisite toggle..."
...
display_versions() {
    log_message "INFO" "Starting version display..."; echo -e "\n${C_CYAN}--- Installed Versions ---${C_RESET}"
...
adjust_php_settings() {
    log_message "INFO" "Starting PHP settings adjustment..."; echo -e "\n${C_MAGENTA}--- Adjust PHP Settings (${PHP_INI_PATH}) ---${C_RESET}"
    ...
    while true; do clear; echo -e "\n${C_MAGENTA}--- Manage PHP Extensions (${PHP_EXTENSIONS_DIR}) ---${C_RESET}"; declare -a ini_files; declare -a statuses; declare -a display_names; declare -a toggleable_flags; local counter=1
        ...
        case $choice in q|Q) echo "Exiting without saving."; log_message "INFO" "PHP ext mgmt aborted."; break ;; 0) echo "Saving and exiting."; if [[ "$changes_made" == true ]]; then echo -e "${C_YELLOW}Restarting PHP-FPM...${C_RESET}"; if run_command "Restart PHP-FPM" systemctl restart php-fpm; then echo -e "${C_GREEN}OK.${C_RESET}"; else echo -e "${C_RED}ERROR!${C_RESET}"; echo -e "${C_YELLOW}Check status/logs.${C_RESET}"; fi; else echo "No changes made."; fi; log_message "INFO" "PHP ext mgmt finished."; break ;; *)
            if [[ "$choice" =~ ^[0-9]+$ ]] && [[ "$choice" -ge 1 ]] && [[ "$choice" -le $num_extensions ]]; then local index=$((choice - 1)); local target_file="${ini_files[$index]}"; local current_status_raw="${statuses[$index]}"; local display_name="${display_names[$index]}"; local toggleable="${toggleable_flags[$index]}"; local base_status; if [[ $current_status_raw == *"Enabled"* ]]; then base_status="Enabled"; elif [[ $current_status_raw == *"Disabled"* ]]; then base_status="Disabled"; else base_status="Unknown"; fi
                if [[ "$toggleable" != "yes" ]]; then echo -e "\n${C_YELLOW}Cannot toggle status for '${display_name}'.${C_RESET}"; echo -e "${C_GREY}(Not toggleable via this menu).${C_RESET}"; sleep 3; continue; fi
...
health_check() {
    ...
    # Check services
    for service in "${services[@]}"; do local service_name="${service}.service"; echo -n "[Service] ${service_name}: Enabled? "; if systemctl is-enabled --quiet "${service_name}"; then echo -e "${C_GREEN}Yes${C_RESET}"; log_message "INFO" "${service_name} enabled."; else echo -e "${C_YELLOW}No${C_RESET}"; log_message "WARN" "${service_name} NOT enabled."; fi; echo -n "[Service] ${service_name}: Active? "; if systemctl is-active --quiet "${service_name}"; then echo -e "${C_GREEN}Yes${C_RESET}"; log_message "INFO" "${service_name} active."; else echo -e "${C_RED}No${C_RESET}"; log_message "WARN" "${service_name} NOT active."; local can_attempt_restart=true; if [[ "$service" == "nginx" && ! -d "/var/log/nginx" ]]; then can_attempt_restart=false; log_message "WARN" "Restart ${service_name} skip: log dir missing."; elif [[ "$service" == "php-fpm" && ! -d "/var/log/php-fpm" ]]; then can_attempt_restart=false; log_message "WARN" "Restart ${service_name} skip: log dir missing."; fi; if [[ "$can_attempt_restart" == true ]]; then echo -e "${C_YELLOW} -> Restarting...${C_RESET}"; if run_command "Restart ${service_name}" --suppress systemctl restart "${service_name}"; then echo -e "${C_GREEN}OK.${C_RESET}"; log_message "INFO" "${service_name} restart OK."; else echo -e "${C_RED}FAIL.${C_RESET}"; log_message "ERROR" "${service_name} restart FAILED."; all_ok=false; issues_found=$((issues_found + 1)); echo -e "${C_YELLOW}  Diagnostics: ${C_CYAN}systemctl status ${service_name} && journalctl -xeu ${service_name}${C_RESET}"; fi; else all_ok=false; if [[ $can_attempt_restart == false ]]; then : else issues_found=$((issues_found + 1)); fi; echo -e "${C_YELLOW} -> Restart skipped.${C_RESET}"; fi; fi; done
...
main_menu() {
     while true; do
        ...
        # Prompt
        read -p "$(echo -e ${PROMPT_COLOR}"   Your choice [1,2,3,P,E,M,C,U,V,H,R,L,0]: "${C_RESET})" choice
