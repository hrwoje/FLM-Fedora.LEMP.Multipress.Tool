...
install_ssl() {
    ...
    dnf install -y nss-tools || error_exit "Failed to install nss-tools"
    ...
    mv mkcert-v1.4.4-linux-amd64 /usr/local/bin/mkcert && chmod +x /usr/local/bin/mkcert || error_exit "Failed to set up mkcert"
    ...
    local files=$(find_cert_files)
    [ -z "$files" ] && error_exit "Certificate files not found"
...
configure_http_redirect() {
    ...
    # Add redirect server block at the beginning of the config
