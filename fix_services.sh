...
fix_directory() {
    ...
    if [ "$(stat -c '%U:%G' "$dir")" != "$user:$group" ]; then
        log_message "Fixing permissions for: $dir" "FIX"
...
check_and_fix_service() {
    ...
    if systemctl is-active --quiet "$service.service"; then
        ...
        local error_log
        error_log=$(journalctl -u "$service.service" -n 50 --no-pager)
        ...
        case "$service" in
            ...
            php-fpm)
                ...
                if grep -q "user = apache" /etc/php-fpm.d/www.conf; then
                    log_message "Fixing PHP-FPM user configuration" "FIX"
...
generate_summary() {
    ...
    if $all_ok; then
        ...
        echo -e "\n${YELLOW}Some services require attention. Check the log file for details: $LOG_FILE${NC}"
    fi
}
